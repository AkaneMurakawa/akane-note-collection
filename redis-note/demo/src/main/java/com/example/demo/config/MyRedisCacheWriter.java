...
public class MyRedisCacheWriter implements RedisCacheWriter {
    ...
    public void clean(String name, byte[] pattern) {
        ...
        execute(name, connection -> {
            ...
            try {
                ...
                Set<byte[]> sets = exeKeys(connection,pattern );
                byte[][] keys = Optional.ofNullable(sets).orElse(Collections.emptySet()).toArray(new byte[0][]);
    ...
    private Set<byte[]>  exeKeys(RedisConnection connection, byte[] pattern) {
            Object nativeConnection = connection.getNativeConnection();
            if (nativeConnection instanceof JedisCluster) {
                return connection.keys(pattern);
            }
            if (nativeConnection instanceof JedisSentinelPool || nativeConnection instanceof RedisProperties.Jedis) {
                Set<byte[]> keySet = Sets.newHashSet();
                Cursor<byte[]> cursor =  connection.scan(ScanOptions.scanOptions().count(REDIS_SCAN_COUNT).match(new String(pattern,StandardCharsets.UTF_8)).build());
